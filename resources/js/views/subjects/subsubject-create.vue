        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer justify-content-between">
                    ...
                    <button type="button" class="btn btn-warning btn-sm" @click="updateSubSubject(parentSubjectId)" :disabled="!isValidCreateForm" v-if="editing && isSubsubject">Update SubSubject</button>
                    ...
                    <button type="button" class="btn btn-warning btn-sm" @click="createSubSubject(parentSubjectId)" :disabled="!isValidCreateForm" v-else-if="isSubsubject">Create Subsubject</button>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            SubjectBus.$on('subject_edit', (subject, categoryId) => {
                this.editing = true
            ...
            this.$parent.$on('subsubject_create', (subjectId) => {
                this.editing = false
                this.isSubsubject = true
                this.parentSubjectId = subjectId
                this.subject = new Subject({})
                this.subject.subject_parent_id = subjectId
                this.subjectForm = new Form(this.subject)
                $('#addUpdateSubject').modal()
            })
            SubjectBus.$on('subsubject_edit', (subject, parentSubjectId) => {
                this.editing = true
                this.isSubsubject = true
                this.subject = new Subject(subject)
                this.subject.subject_parent_id = parentSubjectId
                this.subjectForm = new Form(this.subject)
                this.subjectId = subject.uuid
        ...
        methods: {
            ...
            createSubSubject(parentSubjectId) {
                this.loading = true
                this.subjectForm
                    .post('/subsubjects')
                    .then(subject => {
                        this.loading = false
                        SubjectBus.$emit('subsubject_created', {subject, parentSubjectId})
                        $('#addUpdateSubject').modal('hide')
                    }).catch(error => {
                    this.loading = false
                });
            ...
            updateSubSubject(parentSubjectId) {
                this.loading = true
                this.subjectForm
                    .put(`/subsubjects/${this.subjectId}`, undefined)
                    .then(subject => {
                        this.loading = false
                        SubjectBus.$emit('subsubject_updated', {subject, parentSubjectId})
                        $('#addUpdateSubject').modal('hide')
                    }).catch(error => {
                    this.loading = false
                });
